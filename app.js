...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const infoTitle = document.getElementById('tech-info-title');
    const infoDesc = document.getElementById('tech-info-desc');
    ...
    if (layoutContainer && items.length && typeof gsap !== 'undefined') {
        ...
        const updateInfoPanel = (item) => {
            ...
            if (infoTitle.innerText === title) return; // Skip if already showing

            gsap.to([infoTitle, infoDesc], {
                opacity: 0,
                x: -20,
                duration: 0.2,
                onComplete: () => {
        ...
        const updateCarousel = (calledFromInteraction = false) => {
            layoutContainer.classList.add('is-active');
            items.forEach((item, index) => {
                ...
                if (offset === 0) {
                    gsap.to(item, { x: 0, z: 200, rotateY: 0, scale: 1.2, opacity: 1, duration: 0.6, ease: "power2.out" });
                    item.classList.add('is-active');
                    updateInfoPanel(item); // Always update panel when card becomes center
                } else {
        ...
        items.forEach((item, i) => {
            ...
            item.addEventListener('mouseenter', () => {
                stopAutoRotate();
                currentIndex = i;
                updateCarousel(true); // This now centers the item AND updates the info panel
            });
        ...
        // Initialize state
        layoutContainer.classList.add('is-active');
        updateCarousel(); // Initial call to sync first item
        startAutoRotate();
    }
